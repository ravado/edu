...
function clearAddCategoryForm() {
    var curr_form = $("#frmAddNewCategory");
    curr_form.find($("#newCatTitle")).val('');
...
// ================================== Нажатие на кнопку добавления вопроса ========================================== //
    $(".addQuestion").click(function() {
        if(($("#title").val() == '') || ($(".new-tags").length == 0)) {
...
// ==================================== Нажатие на кнопку 'Отметить все' ============================================ //
    $("#checkAll").click(function() {
        $(".questionId").attr('checked','checked');
    });
// ------------------------------------ Нажатие на кнопку 'Отметить все' -------------------------------------------- //

// ====================================== Нажатие на кнопку 'Снять все' ============================================= //
    $("#uncheckAll").click(function() {
        $(".questionId").removeAttr('checked');
    });
// -------------------------------------- Нажатие на кнопку 'Снять все' --------------------------------------------- //

// ==================================== Нажатие на кнопку 'Удалить все' ============================================= //
    $("#delChecked").click(function() {
        if($(".questionId:checked").length > 0) {
            $("#modDelQuestion").modal();
        } else {
            hints('info','Надо бы выбрать хоть один вопрос, вы так не считаете?');
        }
    });
// ------------------------------------ Нажатие на кнопку 'Удалить все' --------------------------------------------- //
...
// ==================================== Подтверждение удаления вопросов ============================================= //
    $("#delQuestions").click(function() {
        var id_questions = {id:[]}, icon_load;
        $(".questionId:checked").each(function(index) {
            console.log(index);
            $(this).closest('tr').addClass('toDel');
            id_questions.id.push($(this).val());
        });
        icon_load = $('.iconLoading');
        icon_load.show(); // Показываем иконку загрузки
        $.ajax({type:"POST", async:true, data: id_questions, url: "/adm/ahid/delQuestions", dataType:"json",
            success:function(data){
                if(data.status == 'ok') {
                    hints('success','Вопросы удалены');
        ...
        $("#modDelQuestion").modal('hide');
    });
// ------------------------------------ Подтверждение удаления вопросов --------------------------------------------- //

// ============================== Нажатие на иконку удаления одного вопроса ========================================= //
    $(".delQuestion").click(function() {
        $(this).closest('tr').find('.questionId').attr('checked','checked');
        $("#delQuestions").click();
    });
// ------------------------------ Нажатие на иконку удаления одного вопроса ----------------------------------------- //
